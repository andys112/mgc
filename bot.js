...
client.on("message", async function(message) {
    ...
    if (command === "purge" && botConfigs.plugins[0].activated == true) {
        if (!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send("Na to nemáš práva!");
        ...
        if (!deleteCount || deleteCount < 2 || deleteCount > 100) {
            ...
            message.channel.send("Hej vyber číslo 1-200");
            return;
        }
        ...
        message.channel
            .bulkDelete(fetched)
            .catch(error => message.reply("Chyba, prosím, kontaktuj admina!"));
    }
    ...
    if (command === "kick" && botConfigs.plugins[2].activated == true) {
        ...
        if (!message.member.hasPermission("KICK_MEMBERS")) return message.channel.send("Hej! Na tohle nemáš práva!");
        if (bUser.hasPermission("ADMINISTRATOR")) return message.channel.send("Tohohle človíčka, nemůžu kicknout.")
    ...
    if (command === "ban" && botConfigs.plugins[3].activated == true) {
        let bUser = message.guild.member(message.mentions.users.first() || message.guild.members.get(args[0]));
        if (!bUser) return message.channel.send("Nemůžu najít uživatele.");
        let bReason = args.join(" ").slice(22);
        if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("Hej, co to děláš?");
        if (bUser.hasPermission("ADMINISTRATOR")) return message.channel.send("Tenhle človíček nemůže dostat ban!")
    ...
    if (command === "report" && botConfigs.plugins[4].activated == true) {
        ...
        if (!rUser) return message.channel.send("Nenanšla, jsem toho uživatele!");
    ...
    if (command === "tempmute" && botConfigs.plugins[5].activated == true) {
        ...
        if (!tomute) return message.reply("Nenašla jsem to.");
        if (tomute.hasPermission("ADMINISTRATOR")) return message.reply("Tohle je admin! To nemůžeš!");
        if (!message.member.hasPermission("MUTE_MEMBERS")) return message.channel.send("Na to nemáš právo!");
        ...
        let mutetime = args[1];
        if (!mutetime) return message.reply("Pozdě!");

        await (tomute.addRole(muterole.id));
        message.reply(`<@${tomute.id}> byl/a mutenutá na ${ms(mutetime)}`)
    ...
    if (command === "weather" && botConfigs.plugins[7].activated == true) {
        weather.find({ search: args.join(" "), degreeType: botConfigs.weather.degree }, function(err, result) {
            ...
            if (result.length === 0) {
                message.channel.send('**Prosím napište legitimní lokaci! **')
                return;
            }
    ...
    if (command === "lockdown" && botConfigs.plugins[9].activated == true) {
        ...
        if (!time) { return message.reply("Potřebuju čas na jak dlouho to mám locknout!"); }

        const Lockembed = new Discord.RichEmbed()
            .setColor(0xDD2E44)
            .setTimestamp()
            .setTitle("ðŸ”’ LOCKDOWN NOTICE ðŸ”’")
            .setDescription(`Tenhle channel byl uzamčen  ${message.author.tag} for ${time}`);

        const Unlockembed = new Discord.RichEmbed()
            .setColor(0xDD2E44)
            .setTimestamp()
            .setTitle("ðŸ”“ LOCKDOWN NOTICE ðŸ”“")
            .setDescription("Tenhle channel je teď zamčený");
        ...
        if (message.channel.permissionsFor(message.author.id).has("MUTE_MEMBERS") === false) {
            const embed = new Discord.RichEmbed()
                ...
                .setDescription("Nemáš dostatečná práva používat tenhle příkaz!");
    ...
    if (command === "shutdown" && botConfigs.plugins[10].activated == true) {
        if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Hej, tohle není určený Tobě!");
        await message.channel.send(`Dobrou noc, ${message.author.tag}!`);
    ...
    if (command === "announce" && botConfigs.plugins[18].activated === true) {
        ...
        if (!embed_channel) return message.channel.send(`Promiň, tohle nepůjde.Vždyť to ani není channel, nebo je něco blbě!`)
    ...
    if (command === "leave" && botConfigs.plugins[8].activated == true || command === "stop" && botConfigs.plugins[8].activated == true) {
        if (!message.member.voiceChannel) return message.channel.send('Prosím, připoj se do voice channelu.');
        if (!message.guild.me.voiceChannel) return message.channel.send('Promiň, bot asi není aktivní ve správné skupině.');
        if (message.guild.me.voiceChannelID !== message.member.voiceChannelID) return message.channel.send('Promiň, ale nejsi ve stejném voice channelu, jako bot');
        message.guild.me.voiceChannel.leave();
        message.channel.send('Odcházím...');
    }
    ...
    if (command === "pause" && botConfigs.plugins[8].activated == true) {
        ...
        if (message.member.voiceChannel !== message.guild.me.voiceChannel) return message.channel.send('Promiň, ale nejsi ve stejném voice channelu, jako bot');

        if (fetched.dispatcher.paused) return message.channel.send('Vždyť už je to v pauze!');
    ...
    if (command === "play" && botConfigs.plugins[8].activated == true) {
        if (!message.member.voiceChannel) return message.channel.send('Prosím, připoj se do voice channelu.');
        if (!args[0]) return message.channel.send('Promiň, tohle jsem nepoznala. Zkus URL nebo název skladby.');
        ...
        if (!data.dispatcher) play(client, ops, data);
            ...
            message.channel.send(`Přidáno do fronty: ${info.title} |Přidal: ${message.author.tag}`);
        }
    ...
    if (command === "queue" && botConfigs.plugins[8].activated == true) {
        ...
        let resp = `__**Hraje**__\n**${nowPlaying.songTitle}** -- **Přidal:** *${nowPlaying.requester}*\n\n__**Queue**__\n`;
    ...
    if (command === "fskip" && botConfigs.plugins[8].activated == true) {
        ...
        if (!fetched) return message.channel.send("Teď nic nehraje.");

        if (message.member.voiceChannel !== message.guild.me.voiceChannel) return message.channel.send("Nejsi ve stejném kanálu jako bot..");

        if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("Nejsi admin!");
    ...
    if (command === "skip" && botConfigs.plugins[8].activated == true) {
        ...
        if (message.member.voiceChannel !== message.guild.me.voiceChannel) return message.channel.send('Nejsi ve stejném kanále jako bot!');

        let userCount = message.member.voiceChannel.members.size;
        let required = Math.ceil(userCount / 2);

        if (!fetched.queue[0].voteSkips) fetched.queue[0].voteSkips = [];
        if (fetched.queue[0].voteSkips.includes(message.member.id)) return message.channel.send(`Hej už jsi skipoval! ${fetched.queue[0].voteSkips.length}/${required} required.`);

        fetched.queue[0].voteSkips.push(message.member.id);
        ops.active.set(message.guild.id, fetched);

        if (fetched.queue[0].voteSkips.length >= required) {
            message.channel.send('Přeskakuji...');
            return fetched.dispatcher.end();
        }
        message.channel.send(`Přeskakuji ${fetched.queue[0].voteSkips.length}/${required} required`);
    }
});

client.login(botConfigs.token);
console.log("Už jeduuu");


async function play(client, ops, data) {

    client.channels.get(data.queue[0].announceChannel).send(`Teď hraje: ${data.queue[0].songTitle} | Přidáno: ${data.queue[0].requester}`);
...
async function searchYT(client, message, args, ops) {
    search(args.join(' '), function(err, res) {
        if (err) return message.channel.send('Něco se nepodařilo..');
        ...
        resp += `\n Vyber si číslo \`1-${videos.length}\``;
...
async function playYT(client, message, args, ops) {
    ...
    // if (message.guild.me.voiceChannel) return message.channel.send('Tenhle bot už tu je.');

    if (!args[0]) return message.channel.send('Tohle jsem nepochopila. Zkus zadat URL a nebo název!');
    ...
    if (!data.dispatcher) play(client, ops, data);
        ...
        message.channel.send(`Přidáno do fronty: ${info.title} | Přidáno: ${message.author.tag}`);
    }
